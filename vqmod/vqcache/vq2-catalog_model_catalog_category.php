...
$this->db->query("UPDATE " . DB_PREFIX . "category SET views = views +1  WHERE category_id = '".(int)$category_id."'");
	...
	public function getCategories($parent_id = 0) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id)
				LEFT JOIN " . DB_PREFIX . "category_to_customer_group c2cg ON (c.category_id = c2cg.category_id)
			 WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "'
				AND c2cg.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "'
	...
	public function getTotalCategoriesByCategoryId($parent_id = 0) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id)
				LEFT JOIN " . DB_PREFIX . "category_to_customer_group c2cg ON (c.category_id = c2cg.category_id)
