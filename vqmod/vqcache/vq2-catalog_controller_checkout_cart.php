			...
			foreach ($products as $product) {
				...
				if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
			     ...
			     if ($this->config->get('config_enable_admin_product_customer_group_prices') && !$product_special_query->num_rows){
                    $unit_price = $this->tax->calculate($cgprice['price'], $product['tax_class_id'], $this->config->get('config_tax'));
				    } else {
						...
						foreach ($dataRows as $data) {
							if (!empty($data) && count($data) > 0) {
								...
								if (is_numeric($quantity)) {
									...
									$product = $this->model_catalog_product->getProductBySku($barcode);
								...
								if (!$found && is_numeric($quantity)) {
									$json['items_not_found'][] = $data;
								}
							}
						}
