<?php
class ModelExtensionModuleSocategoryslider extends Model {

	public function getImageCategory_categories($category_id) {
	...
	public function getProducts_categories($data = array()) {
...
//			$sql .= " LIMIT " . (int)$data['limit'];
	...
	public function getCategories_son_categories($data = array()) {
		...
		foreach ($query->rows as $result) {
			...
			$product_data[]= array(
				...
				'all_product'   => $this->model_catalog_product->getTotalProducts($filter_data),
				'title'       	=> $category_info['name'],
				'link'          => $this->url->link('product/category', 'path=' . $result['category_id']),
				'image'         => $this->model_tool_image->resize($category_info['image'], $data['width'], $data['height']),
			);
		}
	...
	public function getTotalProducts_categories($data = array()) {
	...
	public function getCategories_son($data = array()) {
		$sql = "SELECT cp.category_id AS category_id, GROUP_CONCAT(cd1.name ORDER BY cp.level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;') AS name, c1.parent_id, c1.sort_order FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "category c1 ON (cp.category_id = c1.category_id) LEFT JOIN " . DB_PREFIX . "category c2 ON (cp.path_id = c2.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd1 ON (cp.path_id = cd1.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd2 ON (cp.category_id = cd2.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c1.category_id = c2s.category_id) WHERE  c1.parent_id = '".$data['category_id']."' AND cd1.language_id = '" . (int)$this->config->get('config_language_id') . "' AND cd2.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c1.status = 1 AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
