...
class ModelExtensionModuleSofiltershopby extends Model {
	public function getAllOptions($product_id){
		...
		$sql = "SELECT DISTINCT pov.option_value_id, od.name AS option_name, o.type AS option_type, ov.image AS opt_value_image,ovd.name AS opt_value_name, o.option_id FROM ".DB_PREFIX."product_option_value AS pov LEFT JOIN ".DB_PREFIX."option_value AS ov ON pov.option_id = ov.option_id AND pov.option_value_id = ov.option_value_id LEFT JOIN ".DB_PREFIX."option_value_description AS ovd ON pov.option_id = ovd.option_id AND pov.option_value_id = ovd.option_value_id LEFT JOIN ".DB_PREFIX."product_option AS po ON pov.option_id = po.option_id AND pov.product_id = po.product_id LEFT JOIN ".DB_PREFIX."option_description AS od ON pov.option_id = od.option_id LEFT JOIN ".DB_PREFIX."option AS o ON pov.option_id = o.option_id WHERE pov.product_id IN (".$product_id.") AND po.required = '1' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getProducts($opt_value_id,$att_value_id,$manu_value_id,$text_search,$minPrice,$maxPrice,$subcate_value_id,$category_id){
		...
		if($att_value_id != "" && count($att_value_id) > 0)
		{
		...
		foreach($query->rows as $result)
		{
			...
			$price = $this->currency->format($this->tax->calculate($data['price'], $data['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
			if ((float)$data['special']) {
				$price = $this->currency->format($this->tax->calculate($data['special'], $data['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
			}
			$price = round(preg_replace("/[^0-9.]/", "", $price)); // Remove character not number
			$data['price_soFilter'] = round($price);
	...
	public function getAllProducts($category_id){
		...
		foreach($query->rows as $result)
		{
			...
			$price = preg_replace("/[^0-9.]/", "", $price); // Remove character not number
			$data['price_soFilter'] = round($price);
			$product_data[] = $data;
		}
		return $product_data;
	...
	public function getAllManufacturerId($product_id) {
		$product_id = implode(",",array_map('intval',$product_id));
		$query = $this->db->query("SELECT DISTINCT p.manufacturer_id, m.name AS manu_name, m.image AS manu_image, m.sort_order AS attribute_sort FROM " . DB_PREFIX . "product AS p LEFT JOIN ".DB_PREFIX."manufacturer AS m ON p.manufacturer_id = m.manufacturer_id WHERE p.product_id IN (" .$product_id. ") AND p.manufacturer_id != '0' ");
		$manu_data = array();
		foreach($query->rows as $item)
		{
