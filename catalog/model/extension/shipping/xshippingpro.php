	...
	public function getQuote($address) {
		...
		foreach($methods as $single_method) {
			...
			if ($xshippingpro['time_start'] != "" && $xshippingpro['time_end']) {
				if ($time < $xshippingpro['time_start'] || $time >= $xshippingpro['time_end']) {
					$status = false; 
			...
			if ($xshippingpro['rate_type']=='flat') {
				...
				if ($xshippingpro['rate_type']=='grand') {
					...
					foreach($xshippingpro['rate_start'] as $index=>$single) {
						$xshippingpro['rate_start'][$index]=$this->currency->format((float)$single, $currency_code, false, false); 
					}

					foreach($xshippingpro['rate_end'] as $index=>$single) {
						$xshippingpro['rate_end'][$index]=$this->currency->format((float)$single, $currency_code, false, false); 
					}
		...
		if ($xshippingpro_debug && $debugging && !$is_quote) { 
