...
class ControllerExtensionModuleSoquickview extends Controller {
	...
	public function index($setting) {
		...
		//$this->document->addStyle('catalog/view/javascript/so_quickview/css/quickview.css');
		
		/*if (!defined ('magnific_popup')){
			$this->document->addStyle('catalog/view/javascript/so_quickview/css/lib.css');
			$this->document->addScript('catalog/view/javascript/so_quickview/js/jquery.magnific-popup.min.js');
			define( 'magnific_popup', 1 );
		}*/
		
		// Get data
		$data['class_suffix'] 			= $setting['class_suffix'];
		if (isset($setting['label_button'][$this->config->get('config_language_id')])) {
			$data['label_text'] 			= html_entity_decode($setting['label_button'][$this->config->get('config_language_id')]['label_text'], ENT_QUOTES, 'UTF-8');
		}else{
			$data['label_text']  		= $setting['label_text'];
		}
		$data['registry'] = $this->registry;
		$data['quick_status'] = '';
		
		if ($this->soconfig != null) {
			$data['quick_status'] = $this->soconfig->get_settings('quick_status');
		}
		if(version_compare(VERSION, '2.1.0.2', '>')) {
			return $this->load->view('extension/module/so_quickview/default.tpl', $data);
		}else{
			$tem_url = $this->config->get('config_template') . '/template/extension/module/so_quickview/default.tpl';
	...
	public function detail() {
		...
		if ($product_info) {
			...
			if ($product_info['image']) {
				...
				$data['thumb'] = $this->model_tool_image->resize('placeholder.png', $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height'));
			}
			...
			$this->response->setOutput($this->load->view('extension/module/so_quickview/product.tpl', $data));
		} else {
			$url = '';
