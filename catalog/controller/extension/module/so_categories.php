	...
	public function index($setting) {
		...
		foreach($_catids as $category_id){
			...
			if ($category_info['image'] != null) {
				...
				$url = file_exists("image/so_categories/images/".$setting['placeholder_path']);
				if ($url) {
					$image_name = "so_categories/images/".$setting['placeholder_path'];
				} else {
		...
		if ($use_cache){
			...
			if (!$_data) {
				$_data = $this->load->view('extension/module/so_categories/default.tpl', $data);
			...
			return $this->load->view('extension/module/so_categories/default.tpl', $data);
		}
