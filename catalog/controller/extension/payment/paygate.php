...
class ControllerExtensionPaymentPaygate extends Controller
{
	public function index()
	{
		...
		if ($order_info) {
			...
			if ($locale) $checksum_source .= $locale;
			if ($country) $checksum_source .= $country;
			if ($email) $checksum_source .= $email;
			if ($payMethod) $checksum_source .= $payMethod;
			if ($payMethodDetail) $checksum_source .= $payMethodDetail;
			if ($notifyUrl) $checksum_source .= $notifyUrl;
			if ($userField1) $checksum_source .= $userField1;
			if ($userField2) $checksum_source .= $userField2;
			if ($userField3) $checksum_source .= $userField3;
			if ($doVault != '') $checksum_source .= $doVault;
			if ($vaultID != '') $checksum_source .= $vaultID;
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paygate.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/paygate.tpl', $data);
		} else {
			return $this->load->view('extension/payment/paygate.tpl', $data);
		}
	}
	...
	public function paygate_return()
	{
		...
		if (isset($this->session->data['order_id'])) {
			...
			$reference      = filter_var($this->session->data['order_id'] . $_SESSION['REFERENCE'], FILTER_SANITIZE_STRING);
			$encryption_key = $this->config->get('paygate_merchant_key');
			...
			if (isset($TRANSACTION_STATUS)) {
				...
				if ($TRANSACTION_STATUS == 0) {
					...
					$orderStatusId = $this->config->get('paygate_success_order_status_id');
					$statusDesc    = 'approved';
				} else if ($TRANSACTION_STATUS == 2) {
					$orderStatusId = $this->config->get('paygate_failed_order_status_id');
					$statusDesc    = 'declined';
				} else if ($TRANSACTION_STATUS == 4) {
					$orderStatusId = $this->config->get('paygate_cancelled_order_status_id');
	...
	public function notify_handler()
	{
		...
		if (!$errors) {
			...
			foreach ($_POST as $key => $val) {
				if ($key == 'PAYGATE_ID') {
					$checkSumParams .= $this->config->get('paygate_merchant_id');
				}
			...
			$checkSumParams .= $this->config->get('paygate_merchant_key');
	...
	public function confirm()
	{
		...
		if ($this->session->data['payment_method']['code'] == 'paygate') {
			...
			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('paygate_order_status_id'), $comment, true);
		}
	}
}