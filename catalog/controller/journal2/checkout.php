...
//        $this->model_journal2_checkout->save();
    ...
    private function registerAccount() {
        ...
        if (version_compare(VERSION, '3', '>=')) {
			$address_id = $this->addAddress($this->getAddressData($this->request->post, 'payment_'));
        ...
        if ($customer_group_info && !$customer_group_info['approval']) {
            ...
            if (Journal2Utils::getProperty($this->request->post, 'shipping_address') != '1') {
                $this->addAddress($this->getAddressData($this->request->post, 'shipping_'));
            }
    ...
    private function addAddress($data) {
    	if (version_compare(VERSION, '3', '>=')) {
			return $this->model_account_address->addAddress($this->customer->getId(), $data);
		}
