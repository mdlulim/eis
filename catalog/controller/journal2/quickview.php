<?php  
class ControllerJournal2Quickview extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->get['path'])) {
			...
			foreach ($parts as $path_id) {
				...
				if ($category_info) {
					$this->data['breadcrumbs'][] = array(
						...
						'href'      => Journal2Utils::link('product/category', 'path=' . $path),
			...
			if ($category_info) {			
				...
				$this->data['breadcrumbs'][] = array(
					'text'      => $category_info['name'],
					'href'      => Journal2Utils::link('product/category', 'path=' . $this->request->get['path']),
					'separator' => $this->language->get('text_separator')
				);
			}
		}
		...
		if (isset($this->request->get['manufacturer_id'])) {
			$this->data['breadcrumbs'][] = array( 
				...
				'href'      => Journal2Utils::link('product/manufacturer'),
				'separator' => $this->language->get('text_separator')
			);	
			...
			if ($manufacturer_info) {	
				$this->data['breadcrumbs'][] = array(
					...
					'href'	    => Journal2Utils::link('product/manufacturer/info', 'manufacturer_id=' . $this->request->get['manufacturer_id'] . $url),
					'separator' => $this->language->get('text_separator')
				);
			}
		}
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			...
			$this->data['breadcrumbs'][] = array(
				...
				'href'      => Journal2Utils::link('product/search', $url),
				'separator' => $this->language->get('text_separator')
			); 	
		}
		...
		if ($product_info) {
			...
			$this->data['breadcrumbs'][] = array(
				...
				'href'      => Journal2Utils::link('product/product', $url . '&product_id=' . $this->request->get['pid']),
				'separator' => $this->language->get('text_separator')
			);			
			...
			$this->document->addLink(Journal2Utils::link('product/product', 'product_id=' . $this->request->get['pid']), 'canonical');
			...
			$this->data['manufacturers'] = Journal2Utils::link('product/manufacturer/info', 'manufacturer_id=' . $product_info['manufacturer_id']);
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				$this->data['price'] = Journal2Utils::currencyFormat($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
			} else {
				$this->data['price'] = false;
			}
						
			if ((float)$product_info['special']) {
				$this->data['special'] = Journal2Utils::currencyFormat($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax')));
			} else {
				$this->data['special'] = false;
			}
			
			if ($this->config->get('config_tax')) {
				$this->data['tax'] = Journal2Utils::currencyFormat((float)$product_info['special'] ? $product_info['special'] : $product_info['price']);
			} else {
				$this->data['tax'] = false;
			}
			...
			foreach ($discounts as $discount) {
				$this->data['discounts'][] = array(
					...
					'price'    => Journal2Utils::currencyFormat($this->tax->calculate($discount['price'], $product_info['tax_class_id'], $this->config->get('config_tax')))
				);
			}
					...
					foreach ($option['option_value'] as $option_value) {
						if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
							...
							$option_value_data[] = array(
								...
								'image'                   => Journal2Utils::resizeImage($this->model_tool_image, $option_value['image'], $this->journal2->settings->get('product_page_options_push_image_width', 30), $this->journal2->settings->get('product_page_options_push_image_height', 30), 'crop'),
								'price'                   => $price,
			...
			$this->data['attribute_groups'] = $this->model_catalog_product->getProductAttributes($this->request->get['pid']);
			
			$this->data['products'] = array();
			
			$results = $this->model_catalog_product->getProductRelated($this->request->get['pid']);
			...
			foreach ($results as $result) {
				...
				$this->data['products'][] = array(
					...
					'href'    	 => Journal2Utils::link('product/product', 'product_id=' . $result['product_id'])
				);
			}	
			...
			if ($product_info['tag']) {		
				...
				foreach ($tags as $tag) {
					$this->data['tags'][] = array(
						'tag'  => trim($tag),
						'href' => Journal2Utils::link('product/search', 'tag=' . trim($tag))
					);
				}
			}
            ...
            if (VERSION === '1.5.6' || VERSION === '1.5.6.1') {
			...
			$this->quickview_template = 'journal2/quickview/quickview.tpl';

			$this->response->setOutput($this->renderView($this->quickview_template));
		} else {

      		$this->data['breadcrumbs'][] = array(
        		'text'      => $this->language->get('text_error'),
				'href'      => Journal2Utils::link('journal2/quickview', '&pid=' . $product_id),
        		'separator' => $this->language->get('text_separator')
      		);			
      		...
      		$this->data['continue'] = Journal2Utils::link('common/home');
			...
			$this->quickview_template = 'error/not_found.tpl';
    ...
    private function index2() {
		...
		if ($product_info) {
			...
			$data['text_login'] = sprintf($this->language->get('text_login'), Journal2Utils::link('account/login', '', 'SSL'), Journal2Utils::link('account/register', '', 'SSL'));
			...
			$data['manufacturers'] = Journal2Utils::link('product/manufacturer/info', 'manufacturer_id=' . $product_info['manufacturer_id']);
			...
			$data['cart_qty'] = (isset($cartProductIds[$result['product_id']])) ? $cartProductIds[$result['product_id']] : 0;
        ...
        $data['labels'] = $this->model_journal2_product->getLabels($product_info['product_id']);
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				$data['price'] = Journal2Utils::currencyFormat($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
			} else {
				$data['price'] = false;
			}

			if ((float)$product_info['special']) {
				$data['special'] = Journal2Utils::currencyFormat($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax')));
			} else {
				$data['special'] = false;
			}

			if ($this->config->get('config_tax')) {
				$data['tax'] = Journal2Utils::currencyFormat((float)$product_info['special'] ? $product_info['special'] : $product_info['price']);
			} else {
				$data['tax'] = false;
			}
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						...
						$product_option_value_data[] = array(
							...
							'option_value_id'         => $option_value['option_value_id'],
							'name'                    => $option_value['name'],
							'image'                   => $option_value['image'] ? Journal2Utils::resizeImage($this->model_tool_image, $option_value['image'], $this->journal2->settings->get('product_page_options_push_image_width', 30), $this->journal2->settings->get('product_page_options_push_image_height', 30), 'crop') : '',
							'price'                   => $price,
							'price_prefix'            => $option_value['price_prefix']
						);
					}
			...
			$data['continue'] = Journal2Utils::link('common/home');
