...
class ControllerModuleJournal2Carousel extends Controller {
    ...
    public function index($setting) {
        ...
        if (in_array($setting['position'], array('top', 'bottom'))) {
            ...
            $this->journal2->settings->set('module_journal2_carousel_' . $setting['module_id'] . '_classes', implode(' ', $this->data['disable_on_classes']));
            $this->journal2->settings->set('module_journal2_carousel_' . $setting['module_id'] . '_video', Journal2Utils::getVideoBackgroundSettings(Journal2Utils::getProperty($module_data, 'video_background.value.text')));
        ...
        $this->data['spacing'] = Journal2Utils::getProperty($module_data, 'spacing', $this->journal2->settings->get('product_grid_item_spacing', '20'));
