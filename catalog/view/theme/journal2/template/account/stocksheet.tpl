  ...
  <div class="row"><?php echo $column_left; ?><?php echo $column_right; ?>
    ...
    <div id="content" class="<?php echo $class; ?>">
      <h1 class="heading-title"><?php echo $heading_title; ?>
         <span><a href="<?php echo $addStocksheetToCart; ?>" class="btn btn-primary" style="float:right;margin-right: 3px;background-color: silver;margin-top: 3px;" onclick="return confirm('Please note that all current cart items will be replaced with contents from this order, should line Items have no stock or are disabled it will not be added to your cart.');">
  <i class="fa fa-repeat"></i> Add sheet to cart </a></span></h1>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-right price"><?php if (!$this->config->get('config_hide_price') && $product['price']) { ?>
              ...
              <?php } ?></td>
              <td class="text-right action <?php echo isset($product['labels']) && is_array($product['labels']) && isset($product['labels']['outofstock']) ? 'outofstock' : ''; ?>"><a onclick="cart.add('<?php echo $product['product_id']; ?>');" data-toggle="tooltip" title="<?php echo $button_cart; ?>" class="btn btn-primary"><i class="fa fa-shopping-cart"></i></a>
