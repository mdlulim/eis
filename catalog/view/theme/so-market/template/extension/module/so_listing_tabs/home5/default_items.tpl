...
<?php }else { ?>
		<div class="ltabs-items-inner  <?php echo  $type_show == 'loadmore' ? $effect : '  ' ;?>">
<?php } ?>
<?php 
	if (!empty($child_items)) {
		...
		foreach ($child_items as $product) {
			...
			//$image_large_first = array_shift($image_large);
			?>
			...
			<div class="item-wrap">
				...
				<div class="col-lg-5 col-md-5 item-first">
					<div class="ltabs-item <?php echo $productInfo;?>">
						<div class="item-inner product-thumb product-layout" style="padding:0;">
							<div class="product-item-container">
										...
										<div class="product-image-container ">
											<a class="lt-image" 
											   ...
											   title="<?php echo $product['name'] ?>">
												<img src="<?php echo  $image_large[0]['popup']?>" alt="<?php echo $product['name'] ?>">
											</a>
										</div>
									<?php }?>
								</div>
								...
								<div class="thumbs-product">
									...
									$limit = 0; foreach($image_large as $child){ $limit++;  ?>
										<div class="item-img" data-src="<?php echo $child['popup']; ?>"><img src="<?php echo $child['thumb']; ?>" alt="image" /></div>
									<?php if($limit == 4) break; } ?>
								</div>
								
								<script type="text/javascript">
									$(function ($) {
										var $tag_id = $('.<?php echo $productInfo; ?>');
										var ua = navigator.userAgent,
										event = (ua.match(/iPad/i)) ? "touchstart" : "click";
										
										$('.thumbs-product .item-img', $tag_id).each(function() {
											
											if($(this).index() == 0) { $(this).addClass('active');}
											$('.thumbs-product .item-img', $tag_id).bind(event, function() {
												$(this).parent().find(".active").removeClass("active");
												$(this).addClass("active");
												var thumb_src = $(this).attr("data-src");
												$('.product-image-container img', $tag_id).attr("src",thumb_src);
											}); 
										});
			...
			<?php if( $i%7 == 2 ) :?>
			<div class="col-lg-7 col-md-7 item-last">	
				<div class="right-container">
				<div class="row">
			<?php endif; ?>
			
			<?php if( $i%7 > 1 || $i%7 == 0 ) : ?>
				<div class="item col-lg-<?php echo floor(12/$nb_column0)?> col-md-<?php echo floor(12/$nb_column1)?> col-sm-<?php echo floor(12/$nb_column2)?> col-xs-<?php echo floor(12/$nb_column3)?>">
					
					<?php 
					if  ($type_show == 'slider' && ($i % $nb_rows == 1 || $nb_rows == 1)) { ?>
						...
						<div class="item-inner product-thumb product-layout" style="padding:0;">
							<div class="product-item-container">
								...
								<div class="right-block">
									...
									<div class="caption">
										<?php if ($display_title) { ?>
											<h4 style=" text-overflow: ellipsis; width:90%;white-space: nowrap; ">
												<a href="<?php echo $product['href'] ?>" 
												   title="<?php echo $product['name'] ?>" target="<?php echo $item_link_target ?>">
												   <?php  echo $product['name_maxlength'];?>
												</a>
					...
					if  ($type_show == 'slider' && ($i % $nb_rows == 0 || $i == $count)) { ?>	
						</div>
					<?php } 
					if ($type_show == 'loadmore'){ ?>
						</div>
