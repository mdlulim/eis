<div class="ltabs-tabs-wrap font-ct">
	...
	<ul class="ltabs-tabs cf list-sub-cat">
		...
		foreach ($list as $tab) 
		{
			if ($type_source == "0") {
				?>
				<li class="ltabs-tab item-cate<?php echo isset($tab['sel']) ? '  tab-sel tab-loaded' : ''; ?> <?php echo ($tab['category_id'] == ('*')) ? ' tab-all' : ''; ?>"
					data-category-id="<?php echo $tab['category_id']; ?>"
					data-active-content-l=".items-category-<?php echo ($tab['category_id'] == "*") ? 'all' : $tab['category_id']; ?>"  
					>
