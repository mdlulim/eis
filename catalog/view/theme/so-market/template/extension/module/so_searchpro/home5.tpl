...
<div id="sosearchpro" class="sosearchpro-wrapper <?php echo $additional_class; ?>">
	...
	<?php if($show_keyword == 1){ ?>
		<div class="so-keyword hidden-sm hidden-md hidden-xs">
			<span class="title-key"><b><?php echo $str_keyword; ?>:</b></span>
			<span class="item-key">
				<?php $dem=0; foreach($list_products as $item){ $dem++;?>
					<a href="<?php echo $item['href'];?>" target="_blank" title="<?php echo $item['nameFull'];?>"><?php echo $item['name_maxlength'];?></a> <?php if($dem< count($list_products)) echo ' | ';?>
				<?php }?>
			</span>
		</div>
	<?php }?>
