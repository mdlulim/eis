...
<script type="text/javascript">
    ...
    function subscribe_newsletter()
    {
        ...
        if(email != "")
        {
            ...
            {
                $('.show-error').remove();
                $('.email').àfter('<span class="show-error" style="color: red;margin-left: 10px"> Invalid Email </span>')
                return false;
            }
            else
            {
                $.ajax({
                    ...
                    success: function(json) {
                        ...
                        if(json.message == "Subscription Successfull") {
                            checkCookie();
                            $('.email').after('<span class="show-error" style="color: #003bb3;margin-left: 10px"> ' + json.message + '</span>');
                            setTimeout(function () {
                                var this_close = $('.popup-close');
