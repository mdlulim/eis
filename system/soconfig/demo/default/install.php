<?php
	...
	if($number_store > 1){
		...
		//$sql ="DROP TABLE IF EXISTS `".DB_PREFIX."so_product_label`"; $this->db->query($sql);
    	...
    	//$sql="CREATE TABLE `".DB_PREFIX."so_product_label`( `id` int(11) UNSIGNED NOT NULL, `product_id` int(11) NOT NULL, `status` tinyint(1) NOT NULL, `show_category` tinyint(1) NOT NULL, `show_product` tinyint(1) NOT NULL, `type` tinyint(1) NOT NULL, `position` varchar(50) NOT NULL, `image` varchar(255) DEFAULT NULL, `text` varchar(100) DEFAULT NULL, `color` char(7) DEFAULT NULL, `bgcolor` char(7) DEFAULT NULL, `fontsize` char(2) DEFAULT NULL, `width` int(3) DEFAULT NULL, `height` int(3) DEFAULT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8"; $this->db->query($sql);
