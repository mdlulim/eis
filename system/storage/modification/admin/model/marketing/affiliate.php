...
class ModelMarketingAffiliate extends Model {
	...
	public function approve($affiliate_id) {
		...
		if ($affiliate_info) {
			...
			$template->data['text_welcome'] = sprintf($template->data['text_approve_welcome'], $template->data['store_name']);
	...
	public function addTransaction($affiliate_id, $description = '', $amount = '', $order_id = 0) {
		...
		if ($affiliate_info) {
			...
			$template = $this->model_extension_mail_template->load('admin.affiliate_transaction');
