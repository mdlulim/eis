...
<script type="text/javascript" src="view/javascript/summernote/opencart.js"></script> 
                    <textarea name="comment" rows="8" id="input-comment" class="form-control summernote"></textarea>
                  </div>
                </div>
              </form>
            </fieldset>
...
$('#history').load('index.php?route=sale/order/history&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&store_id=<?php echo $store_id; ?>');
...
$('#button-history').on('click', function() {
  ...
  if ($("textarea[name='comment']").val().replace(/(<([^>]+)>)/ig,"") == '') {
	...
	$.ajax({
		...
		type: 'post',
		dataType: 'json',
		data: 'order_status_id=' + encodeURIComponent($('select[name=\'order_status_id\']').val()) + '&notify=' + ($('input[name=\'notify\']').prop('checked') ? 1 : 0) + '&override=' + ($('input[name=\'override\']').prop('checked') ? 1 : 0) + '&append=' + ($('input[name=\'append\']').prop('checked') ? 1 : 0) + ($('input[name=\'order_summary\']').prop('checked') ? '&order_summary=1' : '') + ('&order_summary_products=' + $('input[name=\'order_summary_products[]\']:checked').map(function() { return $(this).val(); }).toArray()) + '&emailtemplate_id=' + $('select[name=\'field_template\']').val() + '&comment=' + encodeURIComponent($('textarea[name=\'comment\']').val()),
		beforeSend: function() {
