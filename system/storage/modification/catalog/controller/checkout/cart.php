	...
	public function import() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if (isset($this->request->files['import'] ) && is_uploaded_file($this->request->files['import']['tmp_name'])) {
				# get file path, extension and size
				$file      = $_FILES['import']['tmp_name'];
				$extension = pathinfo($_FILES['import']['name'], PATHINFO_EXTENSION);
				$fileSize  = intval($_FILES['import']['size'] / 1024);

				# check if file extension is allowed/valid
				if(in_array($extension, $allowedExtensions)) {
					# check if file size does not exceed maximum [5MB]
					if ($fileSize <= 5000) {
						...
						$error = 'The selected "'.$_FILES['import']['name'].' ['.$fileSize.' KB]" file exceeds the maximum upload size of 5000KB.';
					}
				} else {
					# invalid file type
