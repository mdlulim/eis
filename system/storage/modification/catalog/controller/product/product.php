...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		$data['dwname'] = array();
		foreach($download_infos as $download_info)
		{
			$did = $download_info['download_id'];
			$dwninfo = $this->model_catalog_product->getProductdownloadfile($did);
			
			$data['dwname'][] = array(
				'name' => $dwninfo['name'],
				'filename'      => $dwninfo['filename'],
				'href'       => $this->url->link('product/product/download', 'download_id=' . $dwninfo['download_id'], true)
			);
		...
		if ($product_info) {
			...
			$data['button_stocksheet'] = $this->language->get('button_stocksheet');
...
'original' => strpos($this->config->get('config_template'), 'journal2') === 0 ? Journal2Utils::resizeImage($this->model_tool_image, $result['image']) : '',
	...
	public function download() {
		...
		$download_info = $this->model_catalog_product->getProductdownloadfile($download_id);
