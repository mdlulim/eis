(function($) {
    ...
    var initDataTables = function () {
        $(document).ready(function() {
            if ($('#productPurchasedReportDataTable').length > 0) {
                var aOptions = {
                    "sDom"           : "<t><'row'<'col-md-12'i p>>",
                    "oLanguage"      : {"sLengthMenu":"_MENU_ ","sInfo":"Showing _START_ to _END_ of _TOTAL_"},
                    "iDisplayLength" : 10
                };
                var oTable        = $('#productPurchasedReportDataTable').DataTable(aOptions);
    ...
    var initReport = function() {
        ...
        $(document).on('click', '#button-filter-reset', function() {
            ...
            var url   = `index.php?route=report/product_purchased&token=${token}`;
            location  = url;
        });
