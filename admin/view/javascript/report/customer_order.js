(function($) {
    ...
    var initDataTables = function () {
        $(document).ready(function() {
            if ($('#customerOrderReportDataTable').length > 0) {
                var aOptions = {
                    "sDom"           : "<t><'row'<'col-md-12'i p>>",
                    "oLanguage"      : {"sLengthMenu":"_MENU_ ","sInfo":"Showing _START_ to _END_ of _TOTAL_"},
                    "iDisplayLength" : 10
                };
                var oTable        = $('#customerOrderReportDataTable').DataTable(aOptions);
    ...
    var initReport = function() {
        ...
        $(document).on('click', '#button-filter-reset', function() {
            ...
            var url   = `index.php?route=report/customer_order&token=${token}`;
            location  = url;
        });
