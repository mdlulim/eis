...
<div id="content">
  ...
  <div class="container-fluid">
    <div class="panel panel-default">
      ...
      <?php if(!isset($_REQUEST['quote_id'])) { ?>                 
...
$('input[name=\'customer\']').autocomplete({
	...
	'select': function(item) { 
		...
		for (i in  item['address']) {
			html += '<option value="' + item['address'][i]['address_id'] + '">' + item['firstname'] + ', ' + item['address'][i]['address_1'] + ', ' + item['address'][i]['city'] + ', ' + item['address'][i]['country'] + '</option>';
		}
		...
		success: function(json) {
			...
			if (json['error']) {
                ...
                var request_2 = request_1.then(function() {
                    $.ajax({
                        url: '<?php echo $catalog; ?>index.php?route=api/voucher/add&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
                        type: 'post',
...
$('#button-coupon').on('click', function() {
	$.ajax({
		url: '<?php echo $catalog; ?>index.php?route=api/coupon&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
		type: 'post',
...
$('#button-voucher').on('click', function() {
	$.ajax({
		url: '<?php echo $catalog; ?>index.php?route=api/voucher&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
		type: 'post',
...
$('#button-reward').on('click', function() {
	$.ajax({
		url: '<?php echo $catalog; ?>index.php?route=api/reward&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
		type: 'post',
...
$('#button-save').on('click', function() {
	if ($('input[name=\'order_id\']').val() == 0) {
		var url = '<?php echo $catalog; ?>index.php?route=api/order/add&token=' + token + '&quote_id=<?php echo $_REQUEST["quote_id"]; ?>&store_id=' + $('select[name=\'store_id\'] option:selected').val();
	} else {
		var url = '<?php echo $catalog; ?>index.php?route=api/order/edit&token=' + token + '&quote_id=<?php echo $_REQUEST["quote_id"]; ?>&store_id=' + $('select[name=\'store_id\'] option:selected').val() + '&order_id=' + $('input[name=\'order_id\']').val();
	}
