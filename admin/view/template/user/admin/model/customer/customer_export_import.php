...
class ModelCustomerCustomerExportImport extends Model {
	...
	public function importCsvCustomerDataInsert($data) { 
		...
		if ($this->config->get( 'customer_export_import_settings_approved' ) && ($data['approved'] != '')) {
			if($data['approved'] == 'TRUE' || $data['approved'] == '=TRUE()' || $data['approved'] == '1' || $data['approved'] == 'True')
			{
				$data['approved'] = 1;
			}
			else
	...
	public function downloadCsv($min,$max) {
		...
		$approvedExportSettings 	? array_push($table_columns, array('name' => 'Approved','required'=>'1','size'=>'15')) : '';
		...
		foreach ($customers as $key => $customer) {
			...
			$approved = ($approvedExportSettings) ? $customer['approved'] : '';
