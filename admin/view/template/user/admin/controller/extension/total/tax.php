...
class ControllerExtensionTotalTax extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/total/tax', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/total/tax', 'token=' . $this->session->data['token'], true);
