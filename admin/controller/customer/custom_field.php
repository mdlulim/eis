...
class ControllerCustomerCustomField extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$lastid = $this->model_customer_custom_field->addCustomField($this->request->post);
	...
	//	echo $filter_type; exit;
		...
		foreach ($results as $result) {
			...
			$data['custom_fields'][] = array(
				...
				'view'            => $this->url->link('customer/custom_field/view', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $result['custom_field_id'] . $url, true)
			);
		}
	...
	protected function getForm() {
		...
		if(isset($this->request->get['custom_field_id']))
		{
			$data['cancel'] = $this->url->link('customer/custom_field', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $this->request->get['custom_field_id'] . $url, true);
		} else {
