...
class ControllerCustomerCustomerInfo extends Controller {
	...
	protected function getCustomercontactFormTab() {
		...
		$data['action'] = $this->url->link('replogic/customer_contact/edit', 'token=' . $this->session->data['token'] . $url, true);
	...
	protected function getVisitsTab() {
		...
		$data['locations'][] = array(
				...
				'view'          => $this->url->link('customer/customer_info/visitsView', 'token=' . $this->session->data['token'] . '&checkin_id=' . $result['checkin_id'] . $url, true)
