	...
	class ControllerSimpleBlogArticle extends Controller {
		...
		public function index() {
            ...
            if($this->checkDatabase()) {
                ...
                $data['install_database'] = $this->url->link('simple_blog/install/installDatabase', 'token=' . $this->session->data['token'] . '&url=' . $url, true);
		...
		public function insert() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
				...
				$this->response->redirect($this->url->link('simple_blog/article', 'token=' . $this->session->data['token'] . $url, true));				
			}
		...
		public function getList() {
			...
			$data['breadcrumbs'] = array();
	   		...
	   		$data['breadcrumbs'][] = array(
				...
				'href'      => $this->url->link('simple_blog/article', 'token=' . $this->session->data['token'] . $url, true),
	      		'separator' => ' :: '
	   		);
			
			$data['insert'] = $this->url->link('simple_blog/article/insert', 'token=' . $this->session->data['token'] . $url, true);
			$data['delete'] = $this->url->link('simple_blog/article/delete', 'token=' . $this->session->data['token'] . $url, true);
			...
			foreach($results as $result) {
				$data['articles'][] = array(
					...
					'edit'          	=> $this->url->link('simple_blog/article/update', 'token=' . $this->session->data['token'] . '&simple_blog_article_id=' . $result['simple_blog_article_id'] . $url, true)
				);		
			}
			...
			$data['sort_article_title'] = $this->url->link('simple_blog/article', 'token=' . $this->session->data['token'] . '&sort=sbad.article_title' . $url, true);
			$data['sort_author_name'] = $this->url->link('simple_blog/article', 'token=' . $this->session->data['token'] . '&sort=sbau.name' . $url, true);
			$data['sort_sortorder'] = $this->url->link('simple_blog/article', 'token=' . $this->session->data['token'] . '&sort=sba.sort_order' . $url, true);
			$data['sort_status'] = $this->url->link('simple_blog/article', 'token=' . $this->session->data['token'] . '&sort=sba.status' . $url, true);
			$data['sort_date_added'] = $this->url->link('simple_blog/article', 'token=' . $this->session->data['token'] . '&sort=sba.date_added' . $url, true);
			...
			$pagination->url = $this->url->link('simple_blog/article', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
    		...
    		$this->response->setOutput($this->load->view('simple_blog/article_list', $data));
		}		
		...
		public function getForm() {
			...
			if (!isset($this->request->get['simple_blog_article_id'])) {
				$data['action'] = $this->url->link('simple_blog/article/insert', 'token=' . $this->session->data['token'] . $url, true);
			} else {
				$data['action'] = $this->url->link('simple_blog/article/update', 'token=' . $this->session->data['token'] . '&simple_blog_article_id=' . $this->request->get['simple_blog_article_id'] . $url, true);
			}

			$data['cancel'] = $this->url->link('simple_blog/article', 'token=' . $this->session->data['token'] . $url, true);
			...
			if (isset($this->request->post['simple_blog_author_id'])) {
				...
				$data['author_name'] = $this->model_simple_blog_author->getAuthorName($article_info['simple_blog_author_id']);
			} else {
				$data['author_name'] = '';
            ...
            $this->response->setOutput($this->load->view('simple_blog/article_form', $data));
		}
