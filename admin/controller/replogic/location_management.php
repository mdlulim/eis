...
class ControllerReplogicLocationManagement extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_replogic_location_management->addScheduleManagement($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_replogic_location_management->editScheduleManagement($this->request->get['appointment_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected'])) {
			foreach ($this->request->post['selected'] as $appointment_id) {
				$this->model_replogic_location_management->deleteAppointment($appointment_id);
			}
	...
	protected function getList() {
		...
		$schedule_management_total = $this->model_replogic_location_management->getTotalScheduleManagement($filter_data, $allaccess, $current_user_id);
		$results = $this->model_replogic_location_management->getScheduleManagement($filter_data, $allaccess, $current_user_id);
		...
		$data['schedule_managements'][] = array(
				...
				'edit'          => $this->url->link('replogic/location_management/edit', 'token=' . $this->session->data['token'] . '&appointment_id=' . $result['appointment_id'] . $url, true)
			);
		}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['appointment_id'])) {
			$data['action'] = $this->url->link('replogic/location_management/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('replogic/location_management/edit', 'token=' . $this->session->data['token'] . '&appointment_id=' . $this->request->get['appointment_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('replogic/location_management', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['appointment_id']) && $this->request->server['REQUEST_METHOD'] != 'POST') {
			$appointment_info = $this->model_replogic_location_management->getappointment($this->request->get['appointment_id']);
	   ...
	   $data['customers'] = $this->model_customer_customer->getCustomers(); ;
		...
		$this->response->setOutput($this->load->view('replogic/location_management_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'replogic/location_management')) {
