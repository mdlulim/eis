...
class ControllerMarketingAffiliate extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('marketing/affiliate/view', 'token=' . $this->session->data['token'] . '&affiliate_id=' . $lastid . $url, true));
		}
		$this->getForm();
	}
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('marketing/affiliate/view', 'token=' . $this->session->data['token'] . '&affiliate_id=' . $this->request->get['affiliate_id'] . $url, true));
		}
		$this->getForm();
	}
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['affiliates'][] = array(
				...
				'approve'      => $approve,
				'unlock'       => $unlock,
				'view'         => $this->url->link('marketing/affiliate/view', 'token=' . $this->session->data['token'] . '&affiliate_id=' . $result['affiliate_id'] . $url, true),
	...
	protected function getForm() {
		...
		if(isset($this->request->get['affiliate_id']))
		{
			$data['cancel'] = $this->url->link('marketing/affiliate/view', 'token=' . $this->session->data['token'] . '&affiliate_id=' . $this->request->get['affiliate_id'] . $url, true);
		} else {
	...
	public function view() {
		...
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		$this->response->setOutput($this->load->view('marketing/affiliate_view', $data));
	}
