...
class ControllerExtensionShippingXshippingpro extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(isset($this->request->post['action']) && $this->request->post['action']=='import') {
				$this->import();
				$this->response->redirect($this->url->link('extension/shipping/xshippingpro', 'token=' . $this->session->data['token'], 'SSL'));
			}
		...
		$data['action'] = $this->url->link('extension/shipping/xshippingpro', 'token=' . $this->session->data['token'], true);
		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=shipping', true);
		$data['export'] = $this->url->link('extension/shipping/xshippingpro/export', 'token=' . $this->session->data['token'], true);
	...
	public function export(){
		...
		if (isset($this->request->post['xshippingpro_sub_group_name'])) {
			$export['xshippingpro_sub_group_name'] = $this->request->post['xshippingpro_sub_group_name'];
		} else {
			$export['xshippingpro_sub_group_name'] = $this->config->get('xshippingpro_sub_group_name');
		}
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && is_uploaded_file($this->request->files['file_import']['tmp_name']) && file_exists($this->request->files['file_import']['tmp_name'])) {
			...
			if($import_data) {
				...
				if(is_array($import_data) && $import_data['xshippingpro_status']) {
					...
					$save['xshippingpro_sub_group_name']=$import_data['xshippingpro_sub_group_name'];
	...
	private function getFormData($data)
	{
		...
		foreach($data['method_data'] as $single_method) {
			...
			if(!isset($method_data['customer_group']))$method_data['customer_group']=array();
			if(!isset($method_data['geo_zone_id']))$method_data['geo_zone_id']=array();
			if(!isset($method_data['product_category']))$method_data['product_category']=array();
			if(!isset($method_data['product_product']))$method_data['product_product']=array();
			if(!isset($method_data['store']))$method_data['store']=array();
			if(!isset($method_data['manufacturer']))$method_data['manufacturer']=array();
			if(!isset($method_data['payment']))$method_data['payment']=array();
			if(!isset($method_data['days']))$method_data['days']=array();
			if(!isset($method_data['rate_start']))$method_data['rate_start']=array();
			if(!isset($method_data['rate_end']))$method_data['rate_end']=array();
			if(!isset($method_data['rate_total']))$method_data['rate_total']=array();
			if(!isset($method_data['rate_block']))$method_data['rate_block']=array();
			if(!isset($method_data['country']))$method_data['country']=array();

			if(!is_array($method_data['customer_group']))$method_data['customer_group']=array();
			if(!is_array($method_data['geo_zone_id']))$method_data['geo_zone_id']=array();
			if(!is_array($method_data['product_category']))$method_data['product_category']=array();
			if(!is_array($method_data['product_product']))$method_data['product_product']=array();
			if(!is_array($method_data['store']))$method_data['store']=array();
			if(!is_array($method_data['manufacturer']))$method_data['manufacturer']=array();
			if(!is_array($method_data['payment']))$method_data['payment']=array();
			if(!is_array($method_data['days']))$method_data['days']=array();
			if(!is_array($method_data['rate_start']))$method_data['rate_start']=array();
			if(!is_array($method_data['rate_end']))$method_data['rate_end']=array();
			if(!is_array($method_data['rate_total']))$method_data['rate_total']=array();
			if(!is_array($method_data['rate_block']))$method_data['rate_block']=array();
			if(!is_array($method_data['country']))$method_data['country']=array();

			if(!isset($method_data['inc_weight']))$method_data['inc_weight']='';

			if(!isset($method_data['dimensional_overfule']))$method_data['dimensional_overfule']='';
			if(!isset($method_data['dimensional_factor']) || !$method_data['dimensional_factor'])$method_data['dimensional_factor']=5000;


			if(!isset($method_data['name']))$method_data['name']=array();
			if(!is_array($method_data['name']))$method_data['name']=array();

			if(!isset($method_data['desc']))$method_data['desc']=array();
			if(!is_array($method_data['desc']))$method_data['desc']=array();

			if(!isset($method_data['customer_group_all']))$method_data['customer_group_all']='';
			if(!isset($method_data['geo_zone_all']))$method_data['geo_zone_all']='';
			if(!isset($method_data['country_all']))$method_data['country_all']='';
			if(!isset($method_data['store_all']))$method_data['store_all']='';
			if(!isset($method_data['manufacturer_all']))$method_data['manufacturer_all']='';
			if(!isset($method_data['postal_all']))$method_data['postal_all']='';
			if(!isset($method_data['coupon_all']))$method_data['coupon_all']='';
			if(!isset($method_data['payment_all']))$method_data['payment_all']='';
			if(!isset($method_data['postal']))$method_data['postal']='';
			if(!isset($method_data['coupon']))$method_data['coupon']='';
			if(!isset($method_data['postal_rule']))$method_data['postal_rule']='inclusive';
			if(!isset($method_data['coupon_rule']))$method_data['coupon_rule']='inclusive';
			if(!isset($method_data['time_start']))$method_data['time_start']='';
			if(!isset($method_data['time_end']))$method_data['time_end']='';
			if(!isset($method_data['rate_final']))$method_data['rate_final']='single';
			if(!isset($method_data['rate_percent']))$method_data['rate_percent']='sub';
			if(!isset($method_data['rate_min']))$method_data['rate_min']='';
			if(!isset($method_data['rate_max']))$method_data['rate_max']='';
			if(!isset($method_data['rate_add']))$method_data['rate_add']='';

			if(!isset($method_data['manufacturer_rule']))$method_data['manufacturer_rule']='2';

			if(!isset($method_data['multi_category']))$method_data['multi_category']='all';   
			if(!isset($method_data['additional']))$method_data['additional']=0;
			if(!isset($method_data['modifier_ignore']))$method_data['modifier_ignore']='';
			if(!isset($method_data['logo']))$method_data['logo']='';
			if(!isset($method_data['group']))$method_data['group']=0;

			if(!isset($method_data['order_total_start']))$method_data['order_total_start']=0;
			if(!isset($method_data['order_total_end']))$method_data['order_total_end']=0;
			if(!isset($method_data['weight_start']))$method_data['weight_start']=0;
			if(!isset($method_data['weight_end']))$method_data['weight_end']=0;
			if(!isset($method_data['quantity_start']))$method_data['quantity_start']=0;
			if(!isset($method_data['quantity_end']))$method_data['quantity_end']=0;
			if(!isset($method_data['mask']))$method_data['mask']='';
			if(!isset($method_data['equation']))$method_data['equation']='';
			
			if(!isset($method_data['option_all']))$method_data['option_all']='';
			if(!isset($method_data['option']))$method_data['option']='1';
			if(!isset($method_data['product_option']))$method_data['product_option']=array();
			if(!is_array($method_data['product_option']))$method_data['product_option']=array();
			
			if(!isset($method_data['hide']))$method_data['hide']=array();
		    if(!is_array($method_data['hide']))$method_data['hide']=array();
		  
		    $method_data['display'] = $data['methods'][$no_of_tab];
			...
			foreach ($data['languages'] as $language) { 
				...
				$inc++;
				$return.='<li '.$active_cls.' ><a href="#language'.$language['language_id'].''.$no_of_tab.'" data-toggle="tab"><img src="'.$data['language_dir'].$language['image'].'" title="'.$language['name'].'" /> '.$language['name'].'</a></li>';
			} 
			$return.='</ul>'
			...
			.'<div class="well well-sm" style="height: 115px; overflow: auto; padding: 0;border-radius: 0;box-shadow: none;background: none;border: none;'.(($method_data['country_all']!='1')?'display:block':'').'">'
			.'<select class="form-control" multiple="true" size="5" name="xshippingpro[country][]">';
			foreach ($data['countries'] as $country) {
				$return.='<option value="'.$country['country_id'].'" '.((in_array($country['country_id'],$method_data['country']))?'selected':'').'>'.$country['name'].'</option>';
			}
