<?php
require_once (DIR_TEMPLATE.'extension'.DIRECTORY_SEPARATOR.'module'.DIRECTORY_SEPARATOR.'so_page_builder'.DIRECTORY_SEPARATOR.'data.php');
require_once (DIR_TEMPLATE.'extension'.DIRECTORY_SEPARATOR.'module'.DIRECTORY_SEPARATOR.'so_page_builder'.DIRECTORY_SEPARATOR.'addElement.php');
require_once (DIR_TEMPLATE.'extension'.DIRECTORY_SEPARATOR.'module'.DIRECTORY_SEPARATOR.'so_page_builder'.DIRECTORY_SEPARATOR.'formField.php');
require_once (DIR_TEMPLATE.'extension'.DIRECTORY_SEPARATOR.'module'.DIRECTORY_SEPARATOR.'so_page_builder'.DIRECTORY_SEPARATOR.'shortcodes-func.php');
class ControllerExtensionModuleSopagebuilder extends Controller {
	...
	public function _getDataModule($module_id){
		...
		if ($module_id != "") {
			$module_info = $default;
			$module_info = array_merge($module_info,$this->model_extension_module->getModule($module_id));
			$data['action'] = $this->url->link('extension/module/so_page_builder', 'token=' . $this->session->data['token'] . '&module_id=' . $module_id, 'SSL');			
	...
	public function _duplicateModule($module_id){
		if($this->_checkPermission()){
			$module_id_arr = $this->model_extension_module_so_page_builder->getModuleId(); // Get module id
			$module_id_dup = $module_id_arr[0]['Auto_increment'];
			$this->model_extension_module_so_page_builder->duplicateModule($module_id);
			/*Duplicate file css for Module Id*/
			$file = '../catalog/view/javascript/so_page_builder/css/style_render_'.$module_id.'.css';
			$new_file = '../catalog/view/javascript/so_page_builder/css/style_render_'. $module_id_dup.'.css';
			copy($file,$new_file);
			$this->session->data['success'] = $this->language->get('text_success_duplicate');
			$this->response->redirect($this->url->link('extension/module/so_page_builder', 'module_id='.$module_id_dup.'&token=' . $this->session->data['token'], 'SSL'));				
		}else{
			$this->session->data['warning'] = $this->language->get('error_permission');
		}	
		return $module_id;
	...
	public function _writeFileCss($module_id,$page_builder){ 
		$css = '.so-page-builder .container-fluid{padding:0; overflow:hidden;}.so-page-builder .container{padding:0; overflow:hidden;}.so-page-builder section{overflow:hidden;}';
	...
	public function _breadcrumbs($module_id){
		...
		if ($module_id == "") {
			...
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/so_page_builder', 'token=' . $this->session->data['token'] . '&module_id=' . $module_id, 'SSL')
			);
		}
	...
	// public function _previewPage(){
		...
		// $return = array();
		// //$return['html'] = $this->load->controller('catalog/extension/module/so_page_builder', $data_page);
