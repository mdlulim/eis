...
class ControllerExtensionModuleSoBasicProducts extends Controller {
	...
	public function index() {
		...
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('extension/module/so_basic_products', 'token=' . $this->session->data['token'], 'SSL'));
		}
		// Get module id new 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/module/so_basic_products', 'module_id='.$module_id.'&token=' . $this->session->data['token'], 'SSL'));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/so_basic_products', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/so_basic_products', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$data['moduletabs'] = $this->model_extension_module->getModulesByCode( 'so_basic_products' );
		$data['link'] = $this->url->link('extension/module/so_basic_products', 'token=' . $this->session->data['token'] . '', 'SSL');
		$data['linkremove'] = $this->url->link('extension/module/so_basic_products&token=' . $this->session->data['token']);
		...
		//=== Theme Custom Code====
		$data['type_layout'] = $this->getLayoutMod('so_basic_products');
		...
		$this->response->setOutput($this->load->view('extension/module/so_basic_products.tpl', $data));
	...
	protected function validate() {
		...
		foreach($languages as $language){
			...
			if ((utf8_strlen($module_description[$language['language_id']]['head_name']) < 3) || (utf8_strlen($module_description[$language['language_id']]['head_name']) > 128)) {
