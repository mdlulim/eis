...
class ControllerExtensionModuleSosearchpro extends Controller {
	...
	// Load breadcrumbs
		...
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('extension/module/so_searchpro', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/module/so_searchpro', 'module_id='.$module_id.'&token=' . $this->session->data['token'], 'SSL'));
		...
		foreach($data['languages'] as $language){
			...
			$keyword = 'module_description['.$language['language_id']."]['str_keyword']";
			if (isset($this->error[$keyword])) {
				$data['error_str_keyword'] = $this->error[$keyword];
				break;
			} else {
				$data['error_str_keyword'] = '';
			}
		}
		if (isset($this->error['limit_keyword'])) {
			$data['error_limit_keyword'] = $this->error['limit_keyword'];
		} else {
			$data['error_limit_keyword'] = '';
		}
		// Save and Stay --------------------------------------------------------------
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/so_searchpro', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/so_searchpro', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$data['moduletabs'] = $this->model_extension_module->getModulesByCode( 'so_searchpro');
		$data['link'] = $this->url->link('extension/module/so_searchpro', 'token=' . $this->session->data['token'] . '', 'SSL');
		$data['linkremove'] = $this->url->link('extension/module/so_searchpro&token=' . $this->session->data['token']);
		// Keyword
		...
		//=== Theme Custom Code====
		$data['type_layout'] = $this->getLayoutMod('so_searchpro');
		$data['type_header'] = $this->getHeader();
		...
		$this->response->setOutput($this->load->view('extension/module/so_searchpro.tpl', $data));
	}
