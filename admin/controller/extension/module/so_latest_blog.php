...
class ControllerExtensionModuleSolatestblog extends Controller {
	...
	public function index() {
		...
		if($this->checkDatabase()) {
			...
			$this->response->setOutput($this->load->view('extension/module/so_latest_blog/notification.tpl', $data));
		} else {
			...
			if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
				...
				$this->response->redirect($this->url->link('extension/module/so_latest_blog', 'token=' . $this->session->data['token'], 'SSL'));
			}
			// Get Module Id new 
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
				...
				if($action == "save_edit") {
					$this->response->redirect($this->url->link('extension/module/so_latest_blog', 'module_id='.$module_id.'&token=' . $this->session->data['token'], 'SSL'));
			...
			if (!isset($this->request->get['module_id'])) {
				$data['action'] = $this->url->link('extension/module/so_latest_blog', 'token=' . $this->session->data['token'], 'SSL');
			} else {
				$data['action'] = $this->url->link('extension/module/so_latest_blog', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
			}
			...
			if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST') || $this->request->server['REQUEST_METHOD'] == 'POST' && !$this->validate() && isset($this->request->get['module_id'])) {
				...
				foreach ($categorys as $category_id) {
					$category_info_1 = $this->model_simple_blog_category->getCategory($category_id);
					$category_info_2 = $this->model_simple_blog_category->getPath($category_id);
			...
			$data['link'] = $this->url->link('extension/module/so_latest_blog', 'token=' . $this->session->data['token'] . '', 'SSL');
			$data['linkremove'] = $this->url->link('extension/module/so_latest_blog&token=' . $this->session->data['token']);

			//--------------------------------Load Data -------------------------------------------
			$data['type_layout'] = $this->getLayoutMod('so_latest_blog');
			...
			$this->response->setOutput($this->load->view('extension/module/so_latest_blog/so_latest_blog.tpl', $data));
