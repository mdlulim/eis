...
class ModelCustomerCustomerExportImport extends Model {
	...
	public function importCsvCustomerDataUpdate($data) {
		...
		if ($this->config->get( 'customer_export_import_settings_password' ) && ($data['password'] != '')) {
			$sql .= "password = '" . $this->db->escape(sha1($data['password'])) . "', ";
		}
	...
	public function downloadCsv() {
		...
		//fputcsv($outstream, array('Column 1', 'Column 2', 'Column 3', 'Column 4', 'Column 5'));
...
foreach ($list as $fields) {
	...
	protected function deleteCategorySEOKeywords() {
		...
		$sql = "ALTER TABLE `".DB_PREFIX."oc_url` AUTO_INCREMENT = $auto_increment;";
		$this->db->query( $sql );
	}
	...
	protected function storeCustomerIntoDatabase( &$customer, $exist_custom_field, $exist_salt, $exist_safe, $exist_token, $exist_code ) {
		$customer_id = $customer['customer_id'];
	...
	protected function uploadCustomers( &$reader, $incremental, &$available_customer_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$this->storeCustomerIntoDatabase( $customer, $exist_custom_field, $exist_salt, $exist_safe, $exist_token, $exist_code );
		}
	}
	...
	protected function validateCustomers( &$reader ) {
		...
		$expected_heading = array_merge( $expected_heading, array( "ip", "status", "approved" ) );
		if ($exist_safe) {
...
//		}
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents("http://www.mhccorp.com/index.php?route=information/message&type=tool_export_import_3_12&language_code=$language_code");
