...
class ModelSettingConfiguration extends Model {
	...
	public function addConfigField($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "config_field SET `name` = '" . $this->db->escape($data['name']) . "', `category` = '" . $this->db->escape($data['category']) . "', `type` = '" . $this->db->escape($data['type']) . "', `length` = '" . (int)$this->db->escape($data['length']) . "', `values` = '" . $this->db->escape($data['values']) . "', `help` = '" . $this->db->escape($data['help']) . "', `section` = '" . $this->db->escape($data['section']) . "'");
